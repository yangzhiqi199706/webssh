...
function wvpRuntimeProbeCommand() {
  return [
    ...
    "printf 'listeners.tcp5060=%s\\n' \"$(case \"$(ss -lnt 2>/dev/null | grep -c ':5060' || true)\" in 0) echo 0;; *) echo 1;; esac)\"",
    "printf 'listeners.http18080=%s\\n' \"$(case \"$(ss -lnt 2>/dev/null | grep -c ':18080' || true)\" in 0) echo 0;; *) echo 1;; esac)\"",
    "printf 'listeners.java_wvp=%s\\n' \"$(pgrep -af 'java.*wvp' 2>/dev/null | grep -c java || true)\"",
    "printf 'datasource.postgresDriver=%s\\n' \"$(case \"$(grep -cE 'driver-class-name:.*postgresql' /opt/wvp/config/*.yml /opt/wvp/config/*.yaml 2>/dev/null | awk '{sum += $NF} END {print sum + 0}' || true)\" in 0) echo 0;; *) echo 1;; esac)\"",
    "printf 'datasource.postgresUrl=%s\\n' \"$(case \"$(grep -cE 'url:.*jdbc:postgresql:' /opt/wvp/config/*.yml /opt/wvp/config/*.yaml 2>/dev/null | awk '{sum += $NF} END {print sum + 0}' || true)\" in 0) echo 0;; *) echo 1;; esac)\"",
    "printf 'datasource.postgresDialect=%s\\n' \"$(case \"$(grep -cE 'database-platform:.*PostgreSQL' /opt/wvp/config/*.yml /opt/wvp/config/*.yaml 2>/dev/null | awk '{sum += $NF} END {print sum + 0}' || true)\" in 0) echo 0;; *) echo 1;; esac)\"",
    "printf 'datasource.mysqlUrl=%s\\n' \"$(case \"$(grep -cE 'url:.*jdbc:mysql:' /opt/wvp/config/*.yml /opt/wvp/config/*.yaml 2>/dev/null | awk '{sum += $NF} END {print sum + 0}' || true)\" in 0) echo 0;; *) echo 1;; esac)\"",
    "printf 'database.wvp_app_connections=%s\\n' \"$( (runuser -u omm -- gsql -d wvp -Atc \"SELECT COUNT(*) FROM pg_stat_activity WHERE datname = 'wvp';\" 2>/dev/null || echo 0) || true)\"",
    "printf 'database.wvp_device_rows=%s\\n' \"$( (runuser -u omm -- gsql -d wvp -Atc 'SELECT COUNT(*) FROM wvp_device;' 2>/dev/null || echo 0) || true)\"",
    "printf 'database.wvp_channel_rows=%s\\n' \"$( (runuser -u omm -- gsql -d wvp -Atc 'SELECT COUNT(*) FROM wvp_channel;' 2>/dev/null || echo 0) || true)\"",
    "printf 'database.wvp_log_rows=%s\\n' \"$( (runuser -u omm -- gsql -d wvp -Atc 'SELECT COUNT(*) FROM wvp_log;' 2>/dev/null || echo 0) || true)\"",
