...
function parseWvpRuntimeProbe(probe) {
  ...
  const legacyInactiveDisabled = get('legacyService.active') === 'inactive' && get('legacyService.enabled') === 'disabled';
  const serviceReady = has('service.active') && has('service.enabled') && isActive(get('service.active')) && isEnabled(get('service.enabled'));
  const legacyReady = legacyInactiveDisabled;
  ...
  return {
    ...
    ready: ready,
    restartAllowed: legacyInactiveDisabled,
    checks: checks,
  };
}
