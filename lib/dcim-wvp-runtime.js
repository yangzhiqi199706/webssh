...
function coreReady(runtime) {
  return Boolean(runtime && runtime.status && runtime.status.ready);
}
...
function registerWvpRuntimeRoutes(app, deps) {
  ...
  app.get('/api/db-manager/opengauss/wvp/runtime-status', async function (req, res) {
  ...
  app.post('/api/db-manager/opengauss/wvp/restart', async function (req, res) {
    ...
    appendLog('wvp/restart code=' + result.restartCode + ' ready=' + result.ok +
